...
class LammpsPotentials:
    ...
    class _PairCoeff:
        ...
        def defined_pairs(self):
            all_pair = []
            for int_s, pre_s in zip(self._interacting_species, self._preset_species):
                pair = []
                for ss in int_s:
                    if ss == "*":
                        pair.append(pre_s)
                ...
                all_pair.extend(
                    np.unique(
                        [sorted([p1, p2]) for p1 in pair[0] for p2 in pair[1]], axis=0
                    ).tolist()
                )
            return all_pair
        ...
        def undefined_pairs(self):
            all_pairs = [
                sorted(s)
                for s in itertools.combinations_with_replacement(self._species, 2)
            ]
