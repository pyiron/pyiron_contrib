...
class Table(LammpsPotentials):
    ...
    def __init__(
        ...
        *chemical_elements,
        ...
        assert self.distance.shape == self.energy.shape == self.force.shape

    @cached_property
    def file_content(self):
        txt = "# DATE: {}\n# Potential file generated by pyiron\n\n".format(
            datetime.today().strftime("%Y-%M-%d")
        )
        txt += "ENTRY\nN {} R {} {}\n\n".format(
            len(self.distance), self.distance.min(), self.distance.max()
        )
        for ii, (d, E, f) in enumerate(zip(self.distance, self.energy, self.force)):
            txt += f"{ii + 1} {d} {E} {f}\n"
        return txt

    @cached_property
    def file_name(self):
        return "pot_" + sha1(self.file_content.encode()).hexdigest() + ".pyiron"

    def export(self):
        self.df["filename"] = [str(Path(".").absolute() / self.file_name)]
        self.df["pair_coeff"] = [self.file_name + " ENTRY"]
        with open(self.file_name, "w") as f:
            f.write(self.file_content)

    def __mul__(self, scale_or_potential):
        self.export()
        return super().__mul___(scale_or_potential=scale_or_potential)

    __rmul__ = __mul__

    def __add__(self, potential):
        self.export()
        return super().__add__(potential=potential)
