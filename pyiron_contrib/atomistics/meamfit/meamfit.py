...
class MeamFit(GenericJob):
    def __init__(self, project, job_name):
        ...
        self._calculation_dataframe = pd.DataFrame({'Job id': [], 'Start config': [], 'End config': [], 'Step': [],
                                                    'Quantity to fit': [], 'Weights': []})
    ...
    def write_input(self):
        ...
        self._write_calc_db(calculation_dataframe=self._calculation_dataframe,
                            file_name="fitdbse",
    ...
    def add_job_to_fitting(self, job_id, time_step_start=0, time_step_end=-1, time_step_delta=10, quantity='E0',
        ...
        if int(job_id) in [int(job) for job in self._calculation_dataframe.index]:
            self._calculation_dataframe.loc[job_id] = pd.Series({'Start config': time_step_start,
                                                                 'End config': time_step_end,
                                                                 'Step': time_step_delta,
                                                                 'Quantity to fit': quantity,
                                                                 'Weights': weight})
        else:
            df_tmp = pd.DataFrame({'Job id': [int(job_id)],
    ...
    def collect_output(self):
        ...
        self._potential_timings_dataframe = self._calculate_std(potential_timings_df,
                                                                self._potential_performance_dataframe)
        self.to_hdf()
    ...
    def from_directory(self, directory):
        if not self.status.finished:
            ...
            self._calculation_dataframe = self._read_calc_db(file_name="fitdbse", cwd=directory)
            self.collect_output()
    ...
    def _read_calc_db(file_name="fitdbse", cwd=None):
        ...
        with open(file_name, 'r') as f:
            content = f.readlines()
        names_lst, range_lst, quantity_lst, weight_lst = zip(*[[part for part in line.split(' ') if part != '']
                                                               for line in content[1:]])
        start_config_lst, end_config_lst = zip(*[[int(step) for step in step_range.split('-')]
                                                 for step_range in range_lst])
        df = pd.DataFrame({'Files': names_lst,
    ...
    def _collect_timings(file_name='bestoptfuncs', cwd=None):
        ...
        df = pd.DataFrame({'File': file_name_lst,
                           'Precision': [float(number.replace('D', 'E')) for number in content_table_re_lst[1]],
    ...
    def _calculate_std(potential_timings_df, potential_performance_df):
        ...
        std_lst = [np.std(potential_performance_df[potential_performance_df['Potential'] == ind]['fitdata'].values -
                          potential_performance_df[potential_performance_df['Potential'] == ind]['truedata'].values)
