...
myc = {
    "r": (1.0, 0.0, 44.0 / 255.0),
    "b": (71.0 / 255.0, 0.0, 167.0 / 255.0),
    "o": (1.0, 180.0 / 255.0, 7.0 / 255.0),
    "g": (0.0 / 255.0, 180.0 / 255.0, 7.0 / 255.0),
}
    ...
    def validate_ready_to_run(self):
        ...
        if (self.input.n_shells is None) and (self.input.cutoff_radius is None):
            raise AttributeError(
                "either <job>.input.n_shells or <job>.input.cutoff_radius must be set"
            )
...
class StaticBondAnalysis(_BondAnalysisParent):
    ...
    def _get_irreducible_bond_vector_per_shell(self, nn, n_bonds_per_shell):
        ...
        all_nn_bond_vectors_list = np.array(
            [
                bonds
                for per_shell_bonds in all_nn_bond_vectors_list
                for bonds in per_shell_bonds
            ]
        )
        return (
            per_shell_irreducible_bond_vectors,
            per_shell_0K_rotations,
            all_nn_bond_vectors_list,
        )
    ...
    @staticmethod
    def _get_bond_indexed_neighbor_list(
        nn, n_bonds_per_shell, all_nn_bond_vectors, structure
    ):
        """
        ...
        for n in n_bonds_per_shell:
            ...
            for i, per_atom_nn in enumerate(nn_vecs_per_shell):
                for vec, ind in zip(per_atom_nn, nn_indices_per_shell[i]):
                    try:
                        j = np.argwhere(
                            np.all(np.isclose(vec, bond_vectors_list), axis=1)
                        )[0, 0]
        ...
        for M_ij in per_shell_bond_indexed_neighbor_list:  # enumerate over shells
            ...
            for j, row in enumerate(M_ij.T):  # enumerate over bonds
                ...
                for i, m_ij in enumerate(row):  # enumerate over atoms
                    per_bond.append(
                        [i + 1, m_ij + 1, j + 1]
                    )  # atom_1_index, atom_2_index, symmetry_op_index
    ...
    def analyze_bonds(self):
        ...
        self.input.cutoff_radius = self._set_cutoff_radius(
        ...
        # run get_neighbors
        self._nn, self._n_bonds_per_shell = self._get_nn(
            structure=self.input.structure, cutoff_radius=self.input.cutoff_radius
        )
        # if n_shells is not set in the input, make sure it is now
        self.input.n_shells = self._set_n_shells(
            nn_distances=self._nn.distances, n_shells=self.input.n_shells
        )

        # get irreducible bond vectors and 0K rotations
        irr_bvs = self._get_irreducible_bond_vector_per_shell(
            nn=self._nn, n_bonds_per_shell=self._n_bonds_per_shell
        )
        (
            self.output.per_shell_irreducible_bond_vectors,
            self.output.per_shell_0K_rotations,
            self._all_nn_bond_vectors,
        ) = irr_bvs
        # get transformations
        ...
        self.output.per_shell_bond_indexed_neighbor_list = (
            self._get_bond_indexed_neighbor_list(
                nn=self._nn,
                n_bonds_per_shell=self._n_bonds_per_shell,
                all_nn_bond_vectors=self._all_nn_bond_vectors,
                structure=self.input.structure,
        ...
        # get bond relations
        self.output.per_shell_bond_relations = self._get_bond_relations_list(
            per_shell_bond_indexed_neighbor_list=self.output.per_shell_bond_indexed_neighbor_list
        )
...
class MDBondAnalysis(_BondAnalysisParent):
    ...
    def validate_ready_to_run(self):
        ...
        static = StaticBondAnalysis(
            project=self.project_hdf5, job_name=self.job_name + "_static"
        )
    ...
    def _get_rho_corr_and_uncorr(x_data, y_data, n_bins=101):
        rho_corr, x_edges, y_edges = np.histogram2d(
            x_data, y_data, (n_bins, n_bins), density=True
        )
        rho_uncorr = np.outer(rho_corr.sum(axis=1), rho_corr.sum(axis=0))
        return (
            rho_corr / rho_corr.sum(),
            rho_uncorr / rho_uncorr.sum(),
            x_edges,
            y_edges,
        )
...
class _Histograms(object):
    ...
    def _check_histogram_bonds(required_data, bond, shell):
        ...
        """
        if bond is None:
            required_data = required_data.reshape(
                -1, 3
            )  # flatten over all bonds in the shell
        else:
    ...
    def get_per_shell_1d_histogram(
        self,
        bond_representation,
        shell=0,
        bond=None,
        ...
        rho, bins = np.histogram(
            required_data[:, axis], bins=n_bins, range=d_range, density=density
        )
        if moment:
            moments = [np.mean(required_data[:, axis])]
            moments += [
                stats.moment(a=required_data[:, axis], moment=i)
    ...
    def get_per_shell_3d_histogram(
        self,
        bond_representation,
        supp_data=None,
        shell=0,
