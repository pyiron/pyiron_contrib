...
class MeanField:
    ...
    def dV1(self, bonds, meshes):
        ...
        gradients = np.gradient(
            V, long_mesh[:, 0, 0], t1_mesh[0, :, 0], t2_mesh[0, 0, :], edge_order=2
        )
        return gradients

    def Vmf_component(self, bonds, rotation, eps=1.0):
        """
        ...
        rotated_bonds = (
            bonds - b_xyz * eps + (self.basis[0] @ rotation) * self.b_0 * eps
        )
        vmf_component = self.V1(rotated_bonds, rotation=rotation)
        return vmf_component

    def Vmf(self, bonds, eps=1.0):
        """
    ...
    def Vmfc(self, bonds, eps=1.0):
        ...
        for i, rot in enumerate(self.rotations):
            factor = 1.0 if (i == 0) else 0.5 if (i != 1) else 0.0
            vmfc += factor * self.Vmf_component(bonds, rotation=rot, eps=eps)
        return vmfc

    def find_linear_correction(self, Veff, long_mesh, temperature=100.0, eps=1.0):
        """
        ...
        solver = root_scalar(f, x0=0.0, x1=0.001, rtol=1e-20)
        lm = solver.root
        return lm

    def get_rho(self, bonds, long_mesh, temperature=100.0, eps=1.0, lm=0.0, Veff=None):
        """
    ...
    def fix_Veff(self, meshes, Veff, temperature=100.0, eps=1.0, fix=[1, 1, 1]):
        """
        ...
        if not np.any(fix):
            ...
            d_t = int(t2_mesh.shape[2] / 2) if (t2_mesh[0, 0, :][0] < 0.0) else 0

            # determine the inflection point of the gradient of the effective potential
            long_grad = np.gradient(
                Veff[:, d_t, d_t], long_mesh[:, d_t, d_t], edge_order=2
            )
            long_grad_fit = CubicSpline(long_mesh[:, d_t, d_t], long_grad)
            fine_long = np.linspace(
                long_mesh[:, d_t, d_t][0], long_mesh[:, d_t, d_t][-1], 10000
            )
            cutoff = (
                fine_long[
                    argrelextrema(long_grad_fit(fine_long), np.greater, order=1)[0][0]
                ]
                - self.b_0 * eps
            )
            if cutoff < 0:
            ...
            if fix[2]:
                sel_1 = db <= cutoff
                trunc_grad = np.gradient(
                    Veff[:, d_t, d_t][sel_1],
                    long_mesh[:, d_t, d_t][sel_1],
                    edge_order=2,
                )
                long_fit = CubicSpline(
                    long_mesh[:, d_t, d_t][sel_1],
                    Veff[:, d_t, d_t][sel_1],
            ...
            if fix[1]:
                sel_2 = (db >= 0.0) & (db <= cutoff)
                t_db = np.concatenate((-np.flip(db[sel_2]), db[sel_2]))
                t_pot = np.concatenate(
                    (np.flip(Veff[:, d_t, d_t][sel_2]), Veff[:, d_t, d_t][sel_2])
                )
    ...
    def get_epsilon_pressure(self, eps=1.0):
        if (np.any(self.energy_list) == None) and (np.any(self.strain_list) == None):
            if isinstance(eps, (int, float)):
                return 0.0, 0.0
            else:
                return np.array([0.0] * len(eps)), np.array([0.0] * len(eps))
        else:
            # md
            ...
            # mf
            u_mf = 6.0 * self.r_potential(strains * self.b_0)
            u_mf_fit_eqn = np.poly1d(np.polyfit(strains, u_mf, deg=4))
            du_mf_fit = u_mf_fit_eqn.deriv(m=1)(eps)
            return -4.0 / (3.0 * eps**2 * (np.sqrt(2) * self.b_0) ** 3) * (
                du_md_fit - du_mf_fit
            ), (u_md_fit_eqn(eps) - u_mf_fit_eqn(eps))

    def find_virial_quant(
        self,
        bonds,
        meshes,
        temperature=100.0,
        ...
        if fix:
            ...
            fixed_Veff = self.fix_Veff(
                meshes=meshes, temperature=temperature, eps=eps, fix=fix, Veff=Veff_w_lm
            )
            rho_1 = self.get_rho(
                bonds=bonds,
                long_mesh=long_mesh,
        ...
        a_dV = (
            b_eps * dV[0] + (t1_mesh * rho_1).sum() * dV[1] + (t2_mesh * rho_1) * dV[2]
        )
        N_by_V = 4.0 / (b_eps * np.sqrt(2)) ** 3
        T_vir = 2.0 / KB * (db_dV * rho_1).sum()
        P_vir = (
            -2.0 * N_by_V * (a_dV * rho_1).sum()
            + self.pressure_offset
            + self.get_epsilon_pressure(eps=eps)[0]
        )
        if return_rho_1:
    ...
    def run_nvt(
        self, bonds, temperature=100.0, eps=1.0, lm=0.0, fix=[1, 1, 1], fix_T=False
    ):
        """
        ...
        if fix_T:
            ...
            def virial(args):
                T_vir, _, b_eps = self.find_virial_quant(
                    bonds=bonds,
                    meshes=meshes,
                    temperature=args[0],
                    eps=eps,
            ...
            def virial(args):
                _, _, b_eps = self.find_virial_quant(
                    bonds=bonds,
                    meshes=meshes,
                    temperature=temperature,
                    eps=eps,
        ...
        virial_q = self.find_virial_quant(
            bonds=bonds,
            meshes=meshes,
        ...
        print(
            "T: {}\nT_eff: {}\nT_vir: {}\nP_vir: {}\neps: {}\nlm: {}\n".format(
                temperature, eff_temp, *virial_q[:2], eps, lm
            )
        )
    ...
    def run_npt(
        ...
        if fix_T:
            ...
            def virial(args):
                T_vir, P_vir, b_eps = self.find_virial_quant(
                    bonds=bonds,
                    meshes=meshes,
                    temperature=args[0],
                    eps=args[1],
                ...
                return [
                    abs(temperature - T_vir),
                    (pressure - P_vir),
                    np.abs(self.b_0 * args[1] - b_eps),
                ]
            ...
            def virial(args):
                _, P_vir, b_eps = self.find_virial_quant(
                    bonds=bonds,
                    meshes=meshes,
                    temperature=temperature,
                    eps=args[0],
    ...
    def get_ah_U(
        ...
        epsilons = 1.0 if epsilons is None else epsilons

        # validate
        bonds, temperatures, pressures, epsilons = self._validate(
            bonds=bonds,
            temperatures=temperatures,
            pressures=pressures,
            epsilons=epsilons,
        ...
        for i, (temp, ev_off) in enumerate(zip(T_virs, ev_offsets)):
            per_bond_energy = (self.V1(bonds=bonds[i]) * rho_1s[i]).sum() - self.V1(
                bonds=b_xyz
            )
            ah_U.append(
                (6 * per_bond_energy + self.energy_offset + ev_off - 1.5 * KB * temp)
                * 1000
            )
    ...
    def get_ah_F(ah_U, temperatures, n_fine_samples=10000):
        ...
        fine_temperatures = np.linspace(
            temperatures[0], temperatures[-1], n_fine_samples, endpoint=True
        )
