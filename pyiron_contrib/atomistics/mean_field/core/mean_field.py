...
class MeanField():
    ...
    def __init__(self, b_0, basis, rotations, r_potential, t1_potential=None, t2_potential=None):
        self.b_0 = b_0
    ...
    def dV1(self, bonds):
        ...
        long_mesh, t1_mesh, t2_mesh = self.get_meshes(bonds)
        gradients = np.gradient(V-V.min(), long_mesh[:, 0, 0], t1_mesh[0, :, 0], t2_mesh[0, 0, :], edge_order=2)
        return gradients
...
#         """
#         Find the linear correction factor (Lagrange multiplier) for the mean-field effective potential. The Lagrange 
#         multiplier constrains the single bond density to give the correct bond, thus preserving volume.
...
#         return lm
    ...
    def get_rho(self, bonds, temperature=100., eps=1., lm=0., Veff=None):
        """
        ...
        Veff -= Veff.min()
        long_mesh, _, _ = self.get_meshes(bonds)
        rho_1 = np.exp(-(Veff+lm*(long_mesh-self.b_0*eps))/KB/temperature)
        return rho_1/rho_1.sum()
    
    def fix_Veff(self, bonds, Veff, temperature=100., eps=1., fix=[1, 1, 1]):
        """
        ...
        return new_Veff-new_Veff.min()

    def find_virial_quant(self, bonds, temperature=100., eps=1., lm=0., fix=[1, 1, 1], return_rho_1=False):
        """
        ...
        """
        long_mesh, t1_mesh, t2_mesh = self.get_meshes(bonds)
        rho_1 = self.get_rho(bonds=bonds, temperature=temperature, eps=eps, lm=lm)
        if fix:
            Veff = -KB*temperature*np.log(rho_1)
            fixed_Veff = self.fix_Veff(bonds=bonds, Veff=Veff, temperature=temperature, eps=eps, fix=fix)
            rho_1 = self.get_rho(bonds=bonds, temperature=temperature, Veff=fixed_Veff, eps=eps, lm=0.)  
        b_eps = (long_mesh*rho_1).sum()
        dV = self.dV1(bonds=bonds)
        db_dV = ((long_mesh-b_eps)*dV[0]+t1_mesh*dV[1]+t2_mesh*dV[2])
        a_dV = b_eps*dV[0] 
        N_by_V = 4./(b_eps*np.sqrt(2))**3 
        T_vir = 2./KB*(db_dV*rho_1).sum()
        P_vir = -2.*N_by_V*(a_dV*rho_1).sum()
        if return_rho_1:
            return T_vir, P_vir, b_eps, rho_1
        return T_vir, P_vir, b_eps
    
    def run_nvt(self, bonds, temperature=100., eps=1., fix=[1, 1, 1]):
        """
        ...
        """
        def virial(args):
            T_vir, _, b_eps = self.find_virial_quant(bonds=bonds, temperature=args[0], eps=eps, lm=args[1], fix=fix)
        ...
        virial_q = self.find_virial_quant(bonds=bonds, temperature=eff_temp, eps=eps, lm=lm, fix=fix, return_rho_1=True)
        print('T: {}\nT_eff: {}\nT_vir: {}\nP_vir: {}\neps: {}\nlm: {}\n'.format(temperature, eff_temp, *virial_q[:2], eps, lm))
        return virial_q[-1], eff_temp, eps, lm
    
    def run_npt(self, bonds, temperature=100., pressure=1e-4, fix=[1, 1, 1]):
        """
        ...
        """
        def virial(args):
            T_vir, P_vir, b_eps = self.find_virial_quant(bonds=bonds, temperature=args[0], eps=args[1], lm=args[2], fix=fix)
    ...
    def get_ah_U(self, bonds, temperatures, pressures=None, epsilons=None, fix=[1, 1, 1]):
        """
        ...
        rho_1, eff_temps, eps, lms = zip(*[self.run_ensemble(bonds=bonds[i], temperature=temp, pressure=pressures[i], 
                                                             eps=epsilons[i], ensemble=ensemble, fix=fix) 
        ...
        P_virs = []
        for i, temp in enumerate(temperatures):
            virial_q = self.find_virial_quant(bonds=bonds[i], temperature=eff_temps[i], eps=eps[i], lm=lms[i], fix=fix)
            ah_U.append((6*(self.V1(bonds=bonds[i])*rho_1[i]).sum()-1.5*KB*virial_q[0])*1000)
            T_virs.append(virial_q[0])
            P_virs.append(virial_q[1])
    ...
    def run_ensemble(self, bonds, temperature, pressure, eps, ensemble, fix):
        """
        ...
        """
        if ensemble == 'nvt':
            return self.run_nvt(bonds=bonds, temperature=temperature, eps=eps, fix=fix)
        elif ensemble == 'npt':
            return self.run_npt(bonds=bonds, temperature=temperature, pressure=pressure, fix=fix)
