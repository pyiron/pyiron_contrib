...
class GleMlip(LammpsMlip):
    
    def __init__(self, project, job_name):
        super(GleMlip, self).__init__(project, job_name)
    ...
    def calc_npt_md(self, temperature=300., stress_tensor=None, n_beads=4, timestep=1., damping_timescale=100., 
                    n_ionic_steps=100, n_print=1, seed=32345, port=31415, A=None, C=None):
        if np.array(stress_tensor).shape != np.eye(3).shape:
            raise ValueError("stress_tensor must be an array of shape (3,3)")
        self.custom_input.temperature = temperature
        self.custom_input.stress_tensor = stress_tensor.flatten().tolist()
    ...
    def write_ipi_xml(self):
        tree = ET.parse(self._templates_directory+ '/gle_template_aniso.xml')
        root = tree.getroot()
        ...
        root[4][2][0][0][2].text = str((self.structure.cell.array.flatten()).tolist())
        ...
        root[4][3][1].text = str(self.custom_input.stress_tensor)
        tree.write(filepath)
    ...
    def write_input(self):
        ...
        super(GleMlip, self).write_input()
    ...
    def collect_rdf(self):
        f=open(self.working_directory + '/ipi_out.' + self.structure.get_chemical_symbols()[0] + self.structure.get_chemical_symbols()[0] + '.rdf.dat', "r")
        lines=f.readlines()
        rdf_r = []
        rdf_g_r = []
        for x in lines:
    ...
    def to_hdf(self, hdf=None, group_name=None):
        super(GleMlip, self).to_hdf(hdf=hdf, group_name=group_name)
    ...
    def from_hdf(self, hdf=None, group_name=None):
        super(GleMlip, self).from_hdf(hdf=hdf, group_name=group_name)
