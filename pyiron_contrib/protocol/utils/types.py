...
class PyironJobTypeRegistryMetaType(ABCMeta):
    ...
    def inject_dynamic_types():
        ...
        if module_name not in sys.modules:
            ...
            getLogger("PyironJobTypeRegistryMetaType").info(
                'Skipping not module "%s" not loaded yet. Please call PyironJobTypeRegistryMetaType.inject_dynamic_types manually'
            )
        else:
            ...
            for (
                origin_module,
                wrapped_name,
            ), tp in PyironJobTypeRegistryMetaType.__wrapped.items():
    ...
    def __init__(cls, name, bases, nmspc):
        ...
        if name in (
            "PyironJobTypeRegistry",
            "Protocol",
            "PyironJobTypeRegistryMetaType",
        ):
            return
        ...
        if "__artificial__" not in nmspc:
            ...
            PyironJobTypeRegistryMetaType.wrapped = (
                PyironJobTypeRegistryMetaType.__wrapped
            )
