...
def match_in_error_log(match_lines, job):
    ...
    match_lines = [m if isinstance(m, LineMatcher) else FullLine(m)
...
class HandyMan:
    ...
    def __init__(self, tools: Union[None, Iterable[RepairTool]] = None,
        ...
        if isinstance(tools, Iterable):
            ...
            raise TypeError("tools must a list of RepairTools or a dict of status to RepairTools.")
    ...
    def find_tool(self, job):
        ...
        # highest comes first
        tools = sorted(self.shed[(job.status.string, job.__name__)],
                       key=lambda tool: -tool.priority) \
              + sorted(self.shed[(job.status.string, "generic")],
                       key=lambda tool: -tool.priority)
        for tool in tools:
            try:
                if tool.match(job):
    ...
    def fix_project(self, project, server_override={}, **kwargs):
        ...
        job_ids = tqdm(project.job_table(**kwargs).query('status.isin(@status_list)').id)
        for jid in job_ids:
            try:
            ...
            try:
                if job.master_id is not None and project.get_job_status(job.master_id) == "running":
                    logger.warning(f"Job {jid}'s master is still running, skipping!")
                    continue
                if job.parent_id is not None and project.get_job_status(job.parent_id) == "running":
...
class TimeoutTool(RepairTool):
    ...
    def fix_inplace(self, job, handyman):
        if job.status.aborted and isinstance(job, GenericMaster):
            ...
            # restart this job
            return atleast_one or job.child_project.get_jobs_status().get('submitted', 0) > 0
...
class AtomisticRepairTool(RepairTool, abc.ABC):
    ...
    def fix(self, old_job: GenericJob, new_job: GenericJob):
        if not self._copy_final_structure and old_job._generic_input['calc_mode'] != 'static':
...
class VaspDisableIsymTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and job.input.incar['ISYM'] != 0 and \
                match_in_error_log([
...
class VaspSubspaceTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and match_in_error_log(PartialLine("ERROR in subspace rotation PSSYEVX"), job)
...
class VaspZbrentTool(VaspTool):
    ...
    def match(self, job):
        if job.input.incar['EDIFF'] <= 1e-6 and job['user/handyman/last'] == 'VaspZbrentTool':
            logger.warning(
    ...
    def fix(self, old_job, new_job):
        ...
        if ediff > 1e-6:
            ...
            new_job.structure = ase_to_pyiron(ase_read(
                    old_job.get_workdir_file("CONTCAR")
            ))
...
class VaspZpotrfTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and match_in_error_log(PartialLine("LAPACK: Routine ZPOTRF failed!"), job)
...
class VaspEddavTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and job.input.incar['ALGO'] != 'All' \
                and match_in_error_log(PartialLine("Error EDDDAV: Call to ZHEGV failed."), job)
...
class VaspMinimizeStepsTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and job.input.incar['IBRION'] != -1 \
...
class VaspTooManyKpointsIsym(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and match_in_error_log([
            ...
            ], job) and job.input.incar['ISYM'] == 0
...
class VaspSetupPrimitiveCellTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and \
                ' internal error in VASP: SETUP_PRIMITIVE_CELL, S_NUM not divisible by NPCELL' in get_job_error_log(job)
...
class VaspMemoryErrorTool(VaspTool):
    ...
    def match(self, job):
        ...
        return super().match(job) and not too_many_cores and match_in_error_log([
...
class VaspEddrmmTool(VaspTool):
    def match(self, job):
        return super().match(job) and match_in_error_log(PartialLine("WARNING in EDDRMM: call to ZHEGV failed, returncode ="), job)
...
class VaspSgrconTool(VaspTool):
    ...
    """
    def match(self, job):
        return super().match(job) and job.input.kpoints[2] == 'Monkhorst_Pack' \
...
class SphinxSymmetryOffTool(RepairTool):
    ...
    def match(self, job):
        return super().match(job) and 'Symmetry inconsistency error: symmetries are no group\n' \
                    in job['sphinx.log']
...
class VaspRhosygSymprecTool(VaspTool):
    ...
    def match(self, job):
        return super().match(job) and match_in_error_log(' RHOSYG internal error: stars are not distinct, try to increase SYMPREC to e.g. ', job)
