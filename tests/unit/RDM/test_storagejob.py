...
class TestStorageJob(TestWithCleanProject):
    ...
    def test_use_s3_storage(self):
        ...
        job = self.project.create.job.StorageJob("test")
        job.use_s3_storage(
            config=aws_credentials, bucket_name=full_bucket, _only_warn=True
        )
        ...
        with self.subTest("Warning only: Occupied storage"):
            with self.assertLogs(job._logger) as log_watcher:
                ...
                self.assertEqual(
                    log_watcher.records[-1].message,
                    "Storage NOT empty - Danger of data loss!",
                )
    ...
    def test_add_files_local(self):
        ...
        self.assertTrue(
            os.path.isfile(os.path.join(job.working_directory, "test_file.txt"))
        )
        ...
        with self.subTest(msg="Test copy present file"):
            with self.assertLogs(logger=job._logger, level="WARN") as w:
                ...
                self.assertEqual(
                    len(w.records),
                    1,
                    msg="Adding the same file again should raise a warning that "
                    "the file was not copied",
                )
                self.assertTrue(
                    "not copied, since already present" in str(w.records[-1].message)
                )
        ...
        with self.subTest(msg="Test copy list of files"):
            ...
            self.assertTrue(
                os.path.isfile(os.path.join(job.working_directory, "test_file2.txt"))
            )
    ...
    def test_add_files_s3(self):
        ...
        with self.subTest(msg="Test copy present file with overwrite=True"):
            job.add_files("tmp_dir/test_file.txt", overwrite=True)
            self.assertEqual(
                job["test_file.txt"].data, [b"Changed"]
            )  # load_file does not decode bytes, yet.
    ...
    def test_reload(self):
        ...
        with self.subTest("local"):
            ...
            self.assertTrue(
                os.path.isfile(
                    os.path.join(reload_local.working_directory, "test_file.txt")
                )
            )
        with self.subTest("s3"):
            ...
            self.assertEqual(
                reload_s3._external_storage.list_nodes(), ["test_file.txt"]
            )
    ...
    def test_remove_files_local(self):
        ...
        with self.subTest("FileNotFoundError"):
            self.assertRaises(
                FileNotFoundError,
                job.remove_files,
                ["test_file.txt", "test_file2.txt" "no_file.txt"],
            )
            self.assertTrue("test_file.txt" in job.files_stored)
            self.assertTrue("test_file2.txt" in job.files_stored)
            self.assertTrue(
