...
class TestElasticTensor(unittest.TestCase):
    ...
    def test_get_higher_order_strains(self):
        ...
        self.assertEqual(
            _get_higher_order_strains(
                strain_lst=epsilon, rotations=np.eye(3).reshape(1, 3, 3)
            ).shape,
            (5, 1),
        )
    ...
    def test_calc_elastic_tensor(self):
        strain = [
            [
                [0.005242761019305993, -0.0012053606628952052, -0.0032546722513198236],
        ...
        stress = [
            [
                [1.1937765570713004, -0.2760427230548313, -0.7461461292223824],
