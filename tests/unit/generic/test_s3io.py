...
aws_credentials_w_bucket = {
    "aws_access_key_id": "fake_access_key",
    "aws_secret_access_key": "fake_secret_key",
    "bucket": MY_BUCKET,
}
credentials = {"access_key": "fake_access_key", "secret_key": "fake_secret_key"}
credentials_w_bucket = {
...
class TestS3IO(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.bucket.put_object(
            Key="any", Body=b"any text", Metadata={"File_loc": "root"}
        )
        cls.bucket.put_object(Key="other", Body=b"any text", Metadata={"File_loc": "/"})
        cls.bucket.put_object(
            Key="other2", Body=b"some text", Metadata={"File_loc": "/"}
        )
        cls.bucket.put_object(
            Key="some/path", Body=b"some text", Metadata={"File_loc": "/some"}
        )
        cls.bucket.put_object(
            Key="some/path_to/any",
            Body=b"any path",
            Metadata={"File_loc": "/some/path_to"},
        )
        cls.bucket.put_object(
            Key="some/path_to/some",
            Body=b"path",
            Metadata={"File_loc": "/some/path_to"},
        )
        cls.bucket.put_object(
            Key="random/location", Body=b"text", Metadata={"File_loc": "/random"}
        )
    ...
    def setUp(self):
        ...
        self.i_o_bucket.put_object(
            Key="other", Body=b"any text", Metadata={"File_loc": "/"}
        )
        self.i_o_bucket.put_object(
            Key="to_be_removed", Body=b"some text", Metadata={"File_loc": "/"}
        )
        self.i_o_bucket.put_object(
            Key="some/path", Body=b"some text", Metadata={"File_loc": "/some"}
        )
    ...
    def test___init__(self):
        ...
        self.assertTrue(s3io._bucket == self.bucket)
        self.assertRaises(
            ValueError, FileS3IO, self.current_dir + "/config_noBucket.json"
        )
        s3io = FileS3IO(
            self.current_dir + "/config_noBucket.json", bucket_name=MY_BUCKET
        )
        self.assertTrue(s3io._bucket == self.bucket)
    ...
    def test_list(self):
        self.assertEqual(
            self.s3io.list_all(),
            {"groups": ["random", "some"], "nodes": ["any", "other", "other2"]},
        )
    ...
    def test_put(self):
        ...
        self.assertEqual(
            self.s3io_io.get_s3_object("some_file.txt")["Body"].read().decode("utf8"),
            "text",
        )
        ...
        self.assertEqual(
            self.s3io_io.get_s3_object("some/some_file.txt")["Body"]
            .read()
            .decode("utf8"),
            "text",
        )
        ...
        self.assertEqual(
            self.s3io_io.get_s3_object("dummy")["Body"].read().decode("utf8"), "text"
        )
        ...
        self.s3io_io.put(
            b"random text", filename="w_metadata", metadata={"random": "metadata"}
        )
    ...
    def test_glob(self):
        self.assertEqual(
            self.s3io.glob("some/path???/*"), ["some/path_to/any", "some/path_to/some"]
        )
